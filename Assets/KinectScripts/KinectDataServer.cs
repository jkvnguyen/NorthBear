...
//		}
		...
		try 
		{
            ...
            int snowlevel = 0;
            int tmp2size = 256;
            byte[] tmp2 = new byte[tmp2size];
            this.GetComponent<WeatherControl2>().snowvalue = snowlevel;
            //statusText.text = snowlevel;
            NetworkEventType snowData = NetworkTransport.Receive(out recHostId, out connectionId, out recChannelId, tmp2, tmp2size, out dataSize, out error);
