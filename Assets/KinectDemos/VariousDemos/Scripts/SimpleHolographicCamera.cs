    ...
    void LateUpdate()
    {
		...
		if (cam) 
		{
			if (headPosValid) 
			{
				...
				left = cam.nearClipPlane * (-screenWidth / 2 - headRelPosition.x) / headRelPosition.z;
				right = cam.nearClipPlane * (screenWidth / 2 - headRelPosition.x) / headRelPosition.z;

				bottom = cam.nearClipPlane * (-screenHeight / 2 - headRelPosition.y) / headRelPosition.z;
				top = cam.nearClipPlane * (screenHeight / 2 - headRelPosition.y) / headRelPosition.z;
